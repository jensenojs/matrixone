...
func TestSendPrepareResponse(t *testing.T) {
	...
	convey.Convey("send Prepare response succ", t, func() {
		...
		st := tree.NewPrepareString(tree.Identifier(getPrepareStmtName(1)), "select ?, 1", buf)
		stmts, err := mysql.Parse(ctx, st.Sql.Get(), 1, buf)
		if err != nil {
			t.Error(err)
		}
	...
	convey.Convey("send Prepare response error", t, func() {
		...
		st := tree.NewPrepareString("stmt1", "select ?, 1", buf)
		stmts, err := mysql.Parse(ctx, st.Sql.Get(), 1, buf)
		if err != nil {
			t.Error(err)
		}
