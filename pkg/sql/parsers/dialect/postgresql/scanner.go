...
func (s *Scanner) scanString(delim uint16, typ int) (int, string) {
	...
	for {
		...
		case delim:
			if s.peek(1) != delim {
				s.skip(1)
				return typ, s.sql[start : s.Pos-1]
			}
			fallthrough

		case '\\':
			var buffer strings.Builder
			buffer.WriteString(s.sql[start:s.Pos])
...
func (s *Scanner) scanStringSlow(buffer *strings.Builder, delim uint16, typ int) (int, string) {
	for {
		...
		if ch != delim && ch != '\\' {
			start := s.Pos
			for ; s.Pos < len(s.sql); s.Pos++ {
				ch = uint16(s.sql[s.Pos])
				if ch == delim || ch == '\\' {
					break
				}
			}
...
func (s *Scanner) scanMySQLSpecificComment() (int, string) {
	...
	_, sql := ExtractMysqlComment(s.sql[start:s.Pos])

	s.MysqlSpecialComment = NewScanner(s.dialectType, sql, s.buf)
