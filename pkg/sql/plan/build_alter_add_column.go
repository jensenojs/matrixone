...
func buildAddColumnAndConstraint(ctx CompilerContext, alterPlan *plan.AlterTable, specNewColumn *tree.ColumnTableDef, colType *plan.Type) (*ColDef, error) {
	...
	if auto_incr && hasDefaultValue {
		return nil, moerr.NewErrInvalidDefault(ctx.GetContext(), specNewColumn.Name.Parts[0].Get())
	}
	if !hasDefaultValue {
		defaultValue, err := buildDefaultExpr(specNewColumn, colType, ctx.GetProcess(), ctx.GetBuffer())
		if err != nil {
...
func findPositionRelativeColumn(ctx context.Context, cols []*ColDef, pos *tree.ColumnPosition) (int, error) {
	...
	if pos.Typ == tree.ColumnPositionFirst {
		...
		for i, col := range cols {
			if col.Name == pos.RelativeColumn.Parts[0].Get() {
				relcolIndex = i
				break
			}
		}
		if relcolIndex == -1 {
			return -1, moerr.NewBadFieldError(ctx, pos.RelativeColumn.Parts[0].Get(), "Columns Set")
		}
