...
func TestBuildAlterView(t *testing.T) {
	...
	stmt1, err := parsers.ParseOne(context.Background(), dialect.MYSQL, sql1, 1, ctx.GetBuffer())
	assert.NoError(t, err)
	...
	stmt2, err := parsers.ParseOne(context.Background(), dialect.MYSQL, sql2, 1, ctx.GetBuffer())
	assert.NoError(t, err)
	...
	stmt3, err := parsers.ParseOne(context.Background(), dialect.MYSQL, sql3, 1, ctx.GetBuffer())
	...
	stmt4, err := parsers.ParseOne(context.Background(), dialect.MYSQL, sql4, 1, ctx.GetBuffer())
	assert.NoError(t, err)
	_, err = buildAlterView(stmt4.(*tree.AlterView), ctx)
	assert.Error(t, err)

	sql5 := "alter view verror as select a from a"
	stmt5, err := parsers.ParseOne(context.Background(), dialect.MYSQL, sql5, 1, ctx.GetBuffer())
	assert.NoError(t, err)
