...
func (s *Scope) CreateTempTable(c *Compile) error {
	...
	dbSource, err := c.tempEngine.Database(c.ctx, dbName, c.proc.TxnOperator)
	if err != nil {
		return err
	}
	tblName := qry.GetTableDef().GetName()
	if _, err := dbSource.Relation(c.ctx, dbName+"-"+tblName); err == nil {
		if qry.GetIfNotExists() {
			return nil
		}
		return errors.New(errno.SyntaxErrororAccessRuleViolation, fmt.Sprintf("temporary table '%s' already exists", tblName))
	...
	if err := dbSource.Create(c.ctx, dbName+"-"+tblName, append(exeCols, exeDefs...)); err != nil {
		return err
	}
